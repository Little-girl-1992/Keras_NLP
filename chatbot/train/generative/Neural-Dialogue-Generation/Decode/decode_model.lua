...
local base_model=require("../Atten/atten")
...
function decode_model:InitialReverse()
    self.MMI_model=require("../Atten/atten");
...
function decode_model:MMI(completed_history)
    ...
    for i=1,self.Word_s:size(1) do
        for j=1,#completed_history[i] do
            num=num+1
            completed_history[i][j].raw_score = completed_history[i][j].score 
	    completed_history[i][j].reverse_score = reverse_score[num]
        ...
        for i=1,self.Word_s:size(1) do
            if completed_history[i]~=nil then
                ...
                for j=1,#completed_history[i] do
                    ...
                    local raw_score = completed_history[i][j].raw_score
                    local reverse_score = completed_history[i][j].reverse_score
                    if self.params.NBest then
                        local index=self.params.batch_size*(batch_n-1)+i;
                        local print_string=index.." "..completed_history[i][j].string:size(2).."|"..current_score.."|"..raw_score.."|"..reverse_score.."|"
                        if self.params.output_source_target_side_by_side then
                ...
                if not self.params.NBest then
                    if self.params.output_source_target_side_by_side then
                        local print_string=self:IndexToWord(self.Source[i][1])
                        print_string=print_string.."|"
                        print_string=print_string..self:IndexToWord(best_output[1]).."| "..best_score
                        open_write_file:write(print_string.."\n");
                        if self.params.PrintOutIllustrationSample and i<=5 then
                            print(print_string)
                        end
